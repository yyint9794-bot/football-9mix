...
// Pages deploy — APK >25MB မတင်ရ (R2 သုံးမည်)
const skipApkBundle =
  process.env.PAGES_DEPLOY === '1' || process.env.SKIP_APK_BUNDLE === '1';

// အရင်စနစ် — public/downloads APK များ app assets ထဲ (Capacitor / in-app download)
const publicDownloads = join(root, 'public', 'downloads');
const downloadsDir = join(root, 'dist', 'downloads');
if (!skipApkBundle && existsSync(publicDownloads)) {
