...
function getPublicOrigin(req) {
    const hostHeader = (typeof req.headers['x-forwarded-host'] === 'string' ? req.headers['x-forwarded-host'] : '') ||
        req.headers.host ||
        'localhost:5173';
    const host = hostHeader.split(',')[0].trim();
    const hostName = host.replace(/:\d+$/, '');
    const forwarded = req.headers['x-forwarded-proto'];
    const protoFromHeader = typeof forwarded === 'string' ? forwarded.split(',')[0].trim() : '';
...
function attachHlsProxy(middlewares) {
    middlewares.use('/api/hls-proxy', async (req, res) => {
        try {
